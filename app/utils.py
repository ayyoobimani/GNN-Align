...
def setup(f):
	...
	lang_file_mapping_path = config_dir + "lang_files.txt2"
...
def read_files(editions):
	...
	for f in editions:
		...
		if fp[-4:] != '.txt':
			fp += ".txt"
		res[f] = {}
		if fp in ['eng-x-bible-mixed.txt', 'fin-x-bible-helfi.txt', 'grc-x-bible-helfi.txt', 'heb-x-bible-helfi.txt']:
			dir = '/mounts/Users/student/ayyoob/Dokumente/code/pbc_utils/data/helfi'
		else:
			dir = corpora_dir 
		if os.path.exists(dir + '/' + fp ):
			with codecs.open(dir + '/' + fp, "r", "utf-8") as fi:
				for l in fi:
					if l[0] == "#":
						continue
					l = l.strip().split("\t")
			...
			LOG.warning(f"file {corpora_dir + fp} not found")
	return res 
