...
def get_row_col_editions(source_edition, target_edition, verse_id, verse_alignments, edition_count, single_edition, align_reader, cols_twice_rows, all_editions=None):
    ...
    for edition in all_editions:
        lang = edition # If all_editions contain just languages instead of actual file names (the case for MT)
        
        if edition != source_edition and edition != target_edition and \
            not (single_edition and (lang in used_langs) ):
            used_langs.add(lang)
...
def get_aligns(re, ce, alignments):
    ...
    raw_align = ''
    # If all_editions contain just languages instead of
    # actual file names (the case for MT)
    rf = re
    cf = ce
...
def get_gdfa_mix_predictions(source_predictions, target_predictions, source_edition, target_edition, verse_alignments, verse_alignments_gdfa, verse_id, prev_res):
    ...
    # hoprfully source and target are already ordered
    s_edition, t_edition = source_edition, target_edition
...
def load_simalign_editions(editions_file):
    ...
    # for MT lang list we have verse counts instead of actual edition names
    if comps[1].isdigit():
        editions = None
...
def main(params):
    ...
    if not params.editions_file == None:
        ...
        # If there are no actual edition names, use language names as editions
        # should be fine as long as "all_editions" contain the names in "verse_alignments"
        if editions == None: 
            editions = {}
            for lang in langs: 
                editions[lang]=lang
