...
def get_row_col_editions(source_edition, target_edition, verse_id, verse_alignments, edition_count, single_edition, align_reader, cols_twice_rows, all_editions=None):
    ...
    for edition in all_editions:
        ...
        if lang != source_lang and lang != target_lang and \
...
def main(params):
    ...
    results_all = {}
    for lang_no, lang in enumerate(langs[2:]):
        used_langs = langs[:2]
        used_langs.append(lang)
        lang_results = {}
    
        used_editions = [editions[lang] for lang in used_langs]
        args = []

        # Currently only supports blinker data
        if base_align == 'simalign':
            all_verse_alignments_inter, all_verse_alignments_gdfa = load_simalign_verse_alignments(all_verses, editions, used_langs)
        for verse_id in all_verses:
            ...
            args.append((params.source_edition, params.target_edition, verse_id, verse_alignments, \
                        ...
                        params.cols_twice_rows, used_editions[:]))
        ...
        for id, verse_id in enumerate(all_verses):
            ...
            for method in methods:
                setup_dict_entry(lang_results, method, {"p_hit_count": 0, "s_hit_count": 0, "total_hit_count": 0, "gold_s_hit_count": 0, "prec": 0, "rec": 0, "f1": 0})
                calc_and_update_score(res_aligns[method], pros[verse_id], surs[verse_id], lang_results[method])

        results_all[editions[lang]] = lang_results
            
    out_file_name = f"{params.base_align_method}_source_{params.source_edition}_target_{params.target_edition}_max_{max(row_counts)-1}_rows_{max(column_counts)-1}_columns_editions_single_edition_{str(params.single_edition)}_{gold_name}_single_lang_results"
    with open(os.path.join(params.save_path, out_file_name + '.txt'), 'w') as f_out:
        for no, edition in enumerate(results_all):
            res = results_all[edition]
            if no == 0:
                f_out.write(f'{methods[0]}\nPrecision: {res[methods[0]]["prec"]:.3f}\nRecall: {res[methods[0]]["rec"]:.3f}\nF1: {res[methods[0]]["f1"]:.3f}')
                f_out.write(f'\n\n{methods[1]}\nPrecision: {res[methods[1]]["prec"]:.3f}\nRecall: {res[methods[1]]["rec"]:.3f}\nF1: {res[methods[1]]["f1"]:.3f}')

            f_out.write(f'\n\nUsed edition: {edition}')
            f_out.write(f'\n{methods[2]}\nPrecision: {res[methods[2]]["prec"]:.3f}\nRecall: {res[methods[2]]["rec"]:.3f}\nF1: {res[methods[2]]["f1"]:.3f}')
