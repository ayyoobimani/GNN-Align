...
def main(params):
    ...
    methods = ['init_gdfa_orig_inter','init_gdfa_orig_gdfa','init_gdfa_gdfa_add_itermax_c[1]_[ratio]0.7', 'init_gdfa_itermax_c[2]_[ratio]0.9']
    ...
    for id, verse_id in enumerate(all_verses):
        ...
        for method in methods:
            ...
            result[method] = (prec, rec, f1, aer, row_count)
        
        inter_ind = sum([int(result["init_gdfa_orig_inter"][2] > lim) for lim in f1_limits])
        gdfa_ind = sum([int(result["init_gdfa_orig_gdfa"][2] > lim) for lim in f1_limits])


        inter_res[inter_ind].append(result)
        gdfa_res[gdfa_ind].append(result)
        all_results.append(result)

    out_file_name = f"{params.base_align_method}_source_{params.source_edition}_target_{params.target_edition}_max_{max(row_counts)-1}_rows_{max(column_counts)-1}_columns_editions_single_edition_{str(params.single_edition)}_{gold_name}_hard_verses"
    with open(os.path.join(params.save_path, out_file_name + '.txt'), 'w') as f_out:
        for i in range(2):
            if i == 0:
                f_out.write(f"Intersection")
                results = inter_res
            elif i == 1:
                f_out.write(f"\n\n\ngdfa")
                results = gdfa_res
            ...
            f_out.write(f"\n\n0 < F1 < {f1_limits[0]}\n")
            f_out.write(f"Number of verses: {len(results[0])}\n")
            f_out.write(f"Avg. number of languages: {np.mean([res[methods[i]][-1]-1 for res in results[0]] ):.1f}\n")
            
            f_out.write(f"\n{methods[i]}\n")
            f_out.write(f"Avg. precision: {np.mean([res[methods[i]][0] for res in results[0]] ):.3}\n")
            f_out.write(f"Avg. recall: {np.mean([res[methods[i]][1] for res in results[0]] ):.3}\n")
            f_out.write(f"Avg. F1: {np.mean([res[methods[i]][2] for res in results[0]] ):.3}\n")
            f_out.write(f"Avg. AER: {np.mean([res[methods[i]][3] for res in results[0]] ):.3}\n")

            f_out.write(f"\n{methods[-2]}\n")
            f_out.write(f"Avg. precision: {np.mean([res[methods[-2]][0] for res in results[0]] ):.3}\n")
            f_out.write(f"Avg. recall: {np.mean([res[methods[-2]][1] for res in results[0]] ):.3}\n")
            f_out.write(f"Avg. F1: {np.mean([res[methods[-2]][2] for res in results[0]] ):.3}\n")
            f_out.write(f"Avg. AER: {np.mean([res[methods[-2]][3] for res in results[0]] ):.3}\n")

            f_out.write(f"\n{methods[-1]}\n")
            f_out.write(f"Avg. precision: {np.mean([res[methods[-1]][0] for res in results[0]] ):.3}\n")
            f_out.write(f"Avg. recall: {np.mean([res[methods[-1]][1] for res in results[0]] ):.3}\n")
            f_out.write(f"Avg. F1: {np.mean([res[methods[-1]][2] for res in results[0]] ):.3}\n")
            f_out.write(f"Avg. AER: {np.mean([res[methods[-1]][3] for res in results[0]] ):.3}\n")


            for no, lim in enumerate(f1_limits[:-1]):
                f_out.write(f"\n\n{f1_limits[no]:.1f} < F1 < {f1_limits[no+1]:.1f}\n")
                f_out.write(f"Number of verses: {len(results[no+1])}\n")
                f_out.write(f"Avg. number of languages: {np.mean([res[methods[i]][-1]-1 for res in results[no+1]] ):.1f}\n")
                
                f_out.write(f"\n{methods[i]}\n")
                f_out.write(f"Avg. precision: {np.mean([res[methods[i]][0] for res in results[no+1]] ):.3}\n")
                f_out.write(f"Avg. recall: {np.mean([res[methods[i]][1] for res in results[no+1]] ):.3}\n")
                f_out.write(f"Avg. F1: {np.mean([res[methods[i]][2] for res in results[no+1]] ):.3}\n")
                f_out.write(f"Avg. AER: {np.mean([res[methods[i]][3] for res in results[no+1]] ):.3}\n")

                f_out.write(f"\n{methods[-2]}\n")
                f_out.write(f"Avg. precision: {np.mean([res[methods[-2]][0] for res in results[no+1]] ):.3}\n")
                f_out.write(f"Avg. recall: {np.mean([res[methods[-2]][1] for res in results[no+1]] ):.3}\n")
                f_out.write(f"Avg. F1: {np.mean([res[methods[-2]][2] for res in results[no+1]] ):.3}\n")
                f_out.write(f"Avg. AER: {np.mean([res[methods[-2]][3] for res in results[no+1]] ):.3}\n")

                f_out.write(f"\n{methods[-1]}\n")
                f_out.write(f"Avg. precision: {np.mean([res[methods[-1]][0] for res in results[no+1]] ):.3}\n")
                f_out.write(f"Avg. recall: {np.mean([res[methods[-1]][1] for res in results[no+1]] ):.3}\n")
                f_out.write(f"Avg. F1: {np.mean([res[methods[-1]][2] for res in results[no+1]] ):.3}\n")
                f_out.write(f"Avg. AER: {np.mean([res[methods[-1]][3] for res in results[no+1]] ):.3}\n")


            f_out.write(f"\n\n{f1_limits[-1]:.1f} < F1 < 1\n")
            f_out.write(f"Number of verses: {len(results[-1])}\n")
            f_out.write(f"Avg. number of languages: {np.mean([res[methods[i]][-1]-1 for res in results[-1]] ):.1f}\n")
            
            f_out.write(f"\n{methods[i]}\n")
            f_out.write(f"Avg. precision: {np.mean([res[methods[i]][0] for res in results[-1]] ):.3}\n")
            f_out.write(f"Avg. recall: {np.mean([res[methods[i]][1] for res in results[-1]] ):.3}\n")
            f_out.write(f"Avg. F1: {np.mean([res[methods[i]][2] for res in results[-1]] ):.3}\n")
            f_out.write(f"Avg. AER: {np.mean([res[methods[i]][3] for res in results[-1]] ):.3}\n")

            f_out.write(f"\n{methods[-2]}\n")
            f_out.write(f"Avg. precision: {np.mean([res[methods[-2]][0] for res in results[-1]] ):.3}\n")
            f_out.write(f"Avg. recall: {np.mean([res[methods[-2]][1] for res in results[-1]] ):.3}\n")
            f_out.write(f"Avg. F1: {np.mean([res[methods[-2]][2] for res in results[-1]] ):.3}\n")
            f_out.write(f"Avg. AER: {np.mean([res[methods[-2]][3] for res in results[-1]] ):.3}\n")
      
            f_out.write(f"\n{methods[-1]}\n")
            f_out.write(f"Avg. precision: {np.mean([res[methods[-1]][0] for res in results[-1]] ):.3}\n")
            f_out.write(f"Avg. recall: {np.mean([res[methods[-1]][1] for res in results[-1]] ):.3}\n")
            f_out.write(f"Avg. F1: {np.mean([res[methods[-1]][2] for res in results[-1]] ):.3}\n")
            f_out.write(f"Avg. AER: {np.mean([res[methods[-1]][3] for res in results[-1]] ):.3}\n")
